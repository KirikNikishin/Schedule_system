...
export default class Study_plan_import extends Component {
  ...
  getSortedDepartments = () => {
    const { departments, sortConfig } = this.state;

    // Преобразуем объект в массив для сортировки
    let entries = Object.entries(departments); // [ [id, department_code], ... ]
  ...
  render() {
    ...
    const departments = this.getSortedDepartments();
