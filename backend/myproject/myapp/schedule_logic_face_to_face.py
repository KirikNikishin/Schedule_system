...
def choose_room(kind, department, auditoriums_buildings):
    # flag = False
    priority = ["Old", "New", "Frezer"]
    rooms_kind_key = {
        "Lection": lambda b: random.choice(["Lection_big", "Lection_small"]),
        "Seminar": lambda b: "Seminar",
        "Lab": lambda b: "Laboratory",
        "Sport": lambda b: "Sport"
    }
    for building in priority:
        rooms_by_kind = auditoriums_buildings["Buildings"][building]
        key = rooms_kind_key.get(kind, lambda b: None)(building)
        if key == "Lection_big" or key == "Lection_small":
            if key in rooms_by_kind and rooms_by_kind[key]:
                room = random.choice(rooms_by_kind[key])
                return room, building
        if key == "Seminar" or key == "Laboratory":
            try:
                department_by_kind = auditoriums_buildings["Buildings"][building][f"{key}"][f"{department}"]
            except KeyError:
                department_by_kind = auditoriums_buildings["Buildings"][building][f"{key}"]["0"]

            try:
                room = random.choice(department_by_kind)
            except IndexError:
                department_by_kind = auditoriums_buildings["Buildings"][building][f"{key}"]["0"]
                room = random.choice(department_by_kind)
...
def choose_room_reboot(kind, auditoriums_buildings):
    # department =
    priority = ["New", "Old", "Frezer"]
    rooms_kind_key = {
        "Lection": lambda b: random.choice(["Lection_big", "Lection_small"]),
        "Seminar": lambda b: "Seminar",
        "Lab": lambda b: "Laboratory",
...
def expand_study_plan(plan, group_name, auditoriums_buildings):
    sessions = []
            ...
            if kind in data:
                ...
                if kind == "Lection" or kind == "Sport":
                    for _ in range(count):
                        ...
                        room, building = choose_room(kind, department, auditoriums_buildings)
...
def merge_lections_sports(sessions, auditoriums_buildings):
    lections = {}
    sports = {}
    other = []
    for s in sessions:
    ...
    for items in lections.values():
        if len(items) > 2:
            room, building = choose_room_reboot("Lection", auditoriums_buildings)
            for s in items:
                s.room = room
                s.building = building
        merged.extend(items)
    ...
    for items in sports.values():
        if len(items) > 2:
            room, building = choose_room_reboot("Sport", auditoriums_buildings)
            for s in items:
                s.room = room
                s.building = building
        merged.extend(items)
                ...
                for time_slot, value in times.items():
                    if value == "-" or value.strip() == "":
                        ...
                        try:
                            ...
                            building = parts[-1]  # корпус
                            auditorium = parts[-2]  # аудитория
...
def generate_face_to_face_schedule():
    start_date, end_date, time_slots, semester_type = load_schedule_settings()
    study_plan = build_study_plan()
    groups = fetch_groups()
    auditoriums_buildings = build_auditoriums_buildings()
    weeks = get_weeks(start_date, end_date)

    all_sessions = []
    for code, group_names in groups.items():
        for group in group_names:
            sessions = expand_study_plan(study_plan[code], group, auditoriums_buildings)
            all_sessions.extend(sessions)

    all_sessions = merge_lections_sports(all_sessions, auditoriums_buildings)
